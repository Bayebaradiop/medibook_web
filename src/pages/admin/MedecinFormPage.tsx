...
const MedecinFormPage = () => {
  ...
  const medecin = id && id !== 'nouveau' ? mockMedecins.find(m => m.id === id) : null;
  const isEdit = !!medecin;

  const [form, setForm] = useState({
    prenom: medecin?.prenom || '', nom: medecin?.nom || '', email: medecin?.email || '',
    telephone: medecin?.telephone || '', specialiteId: medecin?.specialiteId || '', password: '',
  });
  ...
  return (
    <DashboardLayout title={isEdit ? 'Modifier le médecin' : 'Nouveau médecin'}>
      <div className="space-y-6 max-w-3xl">
        ...
        <form onSubmit={e => { e.preventDefault(); toast.success(isEdit ? 'Médecin modifié' : 'Médecin créé'); navigate('/admin/medecins'); }} className="space-y-6">
          <div className="medibook-card flex flex-col items-center">
            <div className="w-24 h-24 rounded-full border-2 border-dashed border-input flex items-center justify-center hover:border-primary transition-colors cursor-pointer">
